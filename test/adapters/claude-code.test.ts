...
describe("ClaudeCodeAdapter", () => {
  ...
  it("emits every canonical kind as a native Claude Code file", () => {
    const base = makeResolvedAsset(FIXTURE_SKILL_DIR);
    const expectedPaths = {
      skill: `.claude/skills/${base.asset.id}/SKILL.md`,
      command: `.claude/commands/${base.asset.id}.md`,
      rule: `.claude/rules/${base.asset.id}.md`,
      workflow: `.claude/commands/${base.asset.id}.md`,
    } as const;

    for (const kind of ["skill", "command", "rule", "workflow"] as const) {
      const output = adapter.emit({ ...base, asset: { ...base.asset, kind } });
      expect(output).toHaveLength(1);
      expect(output[0].path).toBe(expectedPaths[kind]);
      expect(output[0].contents).toContain("DO NOT EDIT — generated by coactl");
    }
  });
});
